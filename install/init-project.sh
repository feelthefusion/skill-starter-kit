...
out = ["#!/usr/bin/env bash",
       ...
       "set -euo pipefail", "", "step() { printf '\\n── %s ───────────────────────────────\\n' \"$1\"; }", ""] + ['# One verify at a time per tree: several sessions (or a Stop hook + a manual run) racing the same', '# node_modules / test database is how "tsc vanished" and flaky suites happen. mkdir is atomic; a', '# lock older than 20 min belongs to a dead run.', 'LOCK=".verify.lock"; waited=0', 'until mkdir "$LOCK" 2>/dev/null; do', '  [ -n "$(find "$LOCK" -maxdepth 0 -mmin +20 2>/dev/null)" ] && { rm -rf "$LOCK"; continue; }', '  [ "$waited" -ge 1200 ] && { echo "verify: another run has held $LOCK for 20 minutes" >&2; exit 1; }', '  [ "$waited" -eq 0 ] && echo "verify: another run is in progress, waiting for it"', '  sleep 5; waited=$((waited + 5))', 'done', 'trap \'rm -rf "$LOCK"\' EXIT'] + [""]
...
out += ['step "dependency audit"             # security-gate layer 4: CVEs + known-malicious (MAL-*) packages',
        ...
        "  echo '⚠ actions lint skipped: uvx not installed (kit installer adds uv).'", "fi", "",
